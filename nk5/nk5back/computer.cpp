...
void CommandComputerResource::run(const std::map<std::string, std::string>& params) const{
	...
	if(strcmp(params.find("COMID")->second.c_str(), "start") == 0) {
	...
	} else if(strcmp(params.find("COMID")->second.c_str(), "poweroff") == 0){
		if(IsPcAlive(id, 1)){
			if(!(ShutdownComputer(id)))
				return SDKError("ShutdownComputer", EBLIB_UNKNOWN);
		}
	} else if(strcmp(params.find("COMID")->second.c_str(), "start_device") == 0){
		...
		// bootMenu
		if(bootMenuValue != 3){
			EnableBootMenuForComputer(id,bEnable);
		}
		// device  ACNT_FLAG_BOOT_MASK 11
		if(bootDeviceValue != 11){
			SetStartBootDevice(id,bootDeviceValue);
		}
...
CommandComputerResource::CommandComputerResource(){
	...
	children.push_back(new PoweronCommand());
}

EndUpdateComputerResource::EndUpdateComputerResource(){
	prefix = "end_update";
}
void EndUpdateComputerResource::run(const std::map<std::string, std::string>& params) const{
	/*if(params.find("COM") == params.end())
		missingParameter("COM");
...
StartUpdateComputerResource::StartUpdateComputerResource(){
	prefix = "start_update";

}
void StartUpdateComputerResource::run(const std::map<std::string, std::string>& params) const{
...
UUIDComputerResource::UUIDComputerResource(){
	...
	children.push_back(new EndUpdateComputerResource());
	children.push_back(new StartUpdateComputerResource());
}
...
void CreateComputerResource::run(const std::map<std::string, std::string>& params) const{
	...
	//byt add start 20160325
	OBJ_COMP info2;
	if(!(GetSpecifiedComputer(nid, &info2, &error)))
		return SDKError("GetSpecifiedComputer", error);

	if(info2.nGrpID != info.nGrpID)
		if(!(MoveComputerToSpecifyGroup(nid, info.nGrpID, &error)))
			return SDKError("MoveComputerToSpecifyGroup", error);
	//byt add end 20160325
...
void PoweronCommand::run(const std::map<std::string, std::string>& params) const{
