...
JSON::Value* BaseDiskComputerToJSON(const COMP_USAGE_LIST& info){
	...
	for(int i=0;i<info.nCountComputer;i++){
		JSON::Object* data = new JSON::Object();
		data->add(new JSON::String("Account"), new JSON::Integer(info.nCountComputer));
		data->add(new JSON::String("Name"), new JSON::String(info.compUsage[i].szCompName));
		data->add(new JSON::String("IPAddress"), new JSON::String(info.compUsage[i].szIP4));
		data->add(new JSON::String("MacAddress"), new JSON::String(info.compUsage[i].szMAC));
		arr -> add(data);
	}
