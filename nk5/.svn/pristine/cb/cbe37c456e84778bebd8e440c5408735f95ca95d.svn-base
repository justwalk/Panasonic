define([
  ...
  'models/task',
  'models/base_disk'
  ], function($, _, Backbone, ModelBinder, Task,Server,BaseDisk){
 BaseDisk = Backbone.View.extend({
  ...
  //alert(this.servers[0].get('IP'));
    this.server = this.options.server;
      ...
      this.model.set('Name', this.server.get('Name'));
    ...
    saveModel: function(e) {
      ...
      if (this.model.isValid) {
        ...
        this.model.save({name: this.server.get('Name'),computer_name:this.server.get('Name'),computer_uuid: this.server.id,IOServer:this.server.get('IOServer')}, {
