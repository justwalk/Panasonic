...
define([
  ...
  'modelBinder'
      ...
      servers.fetch({success:function(erorr,data){ 
        var serverID=parseInt(self.task.get('computer_uuid'));
          ...
          for( var i in data){
            ...
            html+='<option value="'+data[i].ID+'|'+data[i].IP+'">'+data[i].IP+'</option>';
          }
         html+='</select></div></div>';
    ...
    actionTask:function(){
          ...
          }else if(action=='stop'||action=='start_update'||action=='end_update'||action=='action_exam'||action=='poweron'||action=='poweroff'||action=='restart_action'||action=='restart'||action=='menu_switch'||action=='start_device'||action=='auto_update'||action=='start'||action=='send_message'||action=='send_command'||action=='logout'){
            this.ShowClientsView();
          }else if (action=='disk_backup'||action=='disk_resynchronize'||action=='disk_recheck') {
            this.ShowBasedisksView();
          }else if(action=='group_restart'||action=='group_stop'||action=='group_poweron'||action=='group_poweroff'||action=='group_restart_action'||action=='group_menu_switch'||action=='group_start_device'||action=='group_start'||action=='group_send_message'||action=='group_send_command'||action=='group_logout'){
            this.ShowGroupsView()
    ...
    copySave:function(){
          ...
          if(action=='script_exe'||action=='ioserver_restart'){
           var ID_IP=$('#server-select',this.el).val().split("|");
              ...
              task.save({'computer_uuid':ID_IP[0],'name':ID_IP[1],'computer_name':ID_IP[1]},{success: function(model, response) {
            ...
            if( this.computersSelected.length>0){
              ...
              task.set('date',this.getNowFormatDate())
              task.save({'computer_uuid':computer_uuid,'computer_name':name,'name':name},{success: function(model, response) {
                 self.collection.add(model);
            }});
            };
          }else if (action=='disk_backup'||action=='disk_resynchronize'||action=='disk_recheck') {
             var ID_name=$('#storages-select',this.el).val().split("|");
              ...
              var task =new Task(date);
              task.set('date',this.getNowFormatDate())
              task.save({'computer_uuid':ID_name[0],'computer_name':ID_name[1],'name':ID_name[1]},{success: function(model, response) {
                 self.collection.add(model);
              }});
          }else if(action=='group_restart'||action=='group_stop'||action=='group_poweron'||action=='group_poweroff'||action=='group_restart_action'||action=='group_menu_switch'||action=='group_start_device'||action=='group_start'||action=='group_send_message'||action=='group_send_command'||action=='group_logout'){
              ...
              task.set('date',this.getNowFormatDate())
              task.save({'computer_uuid':uuid,'computer_name':computerName,'name':computerName},{success: function(model, response) {
                 self.collection.add(model);
              }});
          }   
    ...
    editSave:function(){
        var self=this;
          ...
          if(action=='script_exe'||action=='ioserver_restart'){
           var ID_IP=$('#server-select',this.el).val().split("|");
            this.task.set({'computer_uuid':ID_IP[0],'computer_name':ID_IP[1],'name':ID_IP[1]});
            ...
            if( this.computersSelected.length>0){
              ...
              this.task.set({'computer_uuid':computer_uuid,'computer_name':name,'name':name})
              this.task.save({},{success: function(model, response) {
                 self.collection.add(model);
            }});
            };
             ...
             var ID_name=$('#storages-select',this.el).val().split("|");
              this.task.set({'computer_uuid':ID_name[0],'computer_name':ID_name[1],'name':ID_name[1]});
              this.task.save({},{success: function(model, response) {
                 self.collection.add(model);
              }});
          }else if(action=='group_restart'||action=='group_stop'||action=='group_poweron'||action=='group_poweroff'||action=='group_restart_action'||action=='group_menu_switch'||action=='group_start_device'||action=='group_start'||action=='group_send_message'||action=='group_send_command'||action=='group_logout'){
              ...
              this.task.set({'computer_uuid':uuid,'computer_name':computerName,'name':computerName});
              this.task.save({},{success: function(model, response) {
                 self.collection.add(model);
              }});
          }   
    ...
    editContentTask:function(){
      var restartServerView = new ContentView({task: this.task});
    ...
    logTask:function(){
      ...
      socket.emit('task:log',{task:this.task}, function(err, log) {
        if(log){
          ...
          $('#logcontent').html(log);
          $('#download-log').attr('href','/logs/download?id='+self.task.get('id')).show();
        }else{
          $('#contentft').width(500);
          $('#logcontent').html(App.msgFactory('no log'));
          $('#download-log').attr('href','/#schedule').hide()
        }
      })
    },
