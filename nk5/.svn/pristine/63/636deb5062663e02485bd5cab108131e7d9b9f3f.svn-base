...
//      
    ...
    saveModel: function(e) {
	  ...
	  var now_time = now.getHours() + ":" + (now.getMinutes() < 10 ? '0'+now.getMinutes() : now.getMinutes()) + ":" + (now.getSeconds() < 10 ? '0'+now.getSeconds() : now.getSeconds());
  	  ...
  	  var message = "Restart After " + second + " seconds";  	  
	  ...
	  if (this.model.isValid) {		
	    this.model.save({computer_uuid: this.computers[0].id, computer_name: this.computers[0].get('name'), action: "restart", time: after_time}, {			
	      success: function(model, response) {
	        self.$el.modal('hide');
	      }
	    });
		this.model.save({computer_uuid: this.computers[0].id, computer_name: this.computers[0].get('name'), action: "send_message", time: now_time, execute: "now", memo: message, command: message}, {			
	      success: function(model, response) {
	        self.$el.modal('hide');
	      }
	    });	
