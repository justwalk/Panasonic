    ...
    var ComputerCommands = Backbone.View.extend({
      ...
      toggleCommandAvailability: function() {
        if (this.getActiveComputers()) {
          ...
          if(Number($('#group-select').val()) === -1){
            $(':not(.add-computer) button', this.el).attr('disabled', 'disabled');
            $('.edit-computersT button', this.el).removeAttr('disabled');
            $('.edit-bootMenuT button', this.el).removeAttr('disabled');
            $('.add-computer', this.el).hide();
            $('.edit-computersT', this.el).show();
            $('.edit-bootMenuT', this.el).show();
            $('#context-commands', self.el).hide();
          }else{
            $('.edit-computersT', this.el).hide();
            $('.edit-bootMenuT', this.el).hide();
            $('#context-commands', self.el).show();
          }
        } else {
          if(Number($('#group-select').val()) === -1){
            $('.add-computer', this.el).hide();
