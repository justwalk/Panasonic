define([
  ...
  ], function($, _, Backbone, ModelBinder,ioserverTemlate,scriptexeTemlate,diskcopyTemlate,diskrecheckTemlate,poweroffTemlate,startdeviceTemlate,actionexamTemlate,menusUpdateModeTemplate,menuswitchTemplate,autoupdateTemplate,diskbackupTemplate,sendcmdTemplate,Menus,Task,Server,BaseServer){
 BaseServer = Backbone.View.extend({
     ...
     ,       execute:[
        ...
        this.template=_.template(underi18n.template(menuswitchTemplate, msgFactory));
          ...
          this.menus.fetch({ data: {computer_id: this.model.get('computer_uuid')}});
           ...
           this.template=_.template(underi18n.template(sendcmdTemplate, msgFactory));
           this.bindings = {
           name: '[name=computer_name]',
            memo: '[name=memo]',
            execute:[
    ...
    saveModel: function(e) {
      e.preventDefault();
      var self = this;
      if(this.model.get('action')=='poweroff'||this.model.get('action')=='group_poweroff'||this.model.get('action')=='restart_action'||this.model.get('action')=='group_restart_action'){
          var command=this.model.get('command').split(" ");
