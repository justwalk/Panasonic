...
Task.prototype.writeFile=function(str){
 ...
 Task.deleteFile  = function(task,callback){
    TaskLogFile.all({where:{taskid:task.id}},function(err,data){
      if(data.length > 0){
        data.forEach(function(taskLogFile){
          if(fs.existsSync(config.readlogPath(taskLogFile.folder,"Task-"+task.id))){
            fs.unlink(config.readlogPath(taskLogFile.folder,"Task-"+task.id),function(err){
              if(err){
                console.log(err);
              }
            })
 ...
 Task.queryDeleteByDate=function(dateStr,rc){
    if(dateStr){
      ...
      var sql="DELETE FROM" +'"'+this.modelName+'"'+ "WHERE to_timestamp(date,'dd/mm/yyyy') < to_timestamp('"+dateStr+"','dd/mm/yyyy') and next_execution_date is null";
      var taskLogFilesql="DELETE FROM" +'"TaskLogFile"'+ "WHERE to_timestamp(folder,'yyyymmdd') < to_timestamp('"+dataformat+"','yyyymmdd')";
     // var taskLogFilesql="DELETE FROM" +'"TaskLogFile"'+ "WHERE folder < '"+dataformat+"'";
      client.query(sql,function(err,a){
        rc(err,a);
      })
      client.query(taskLogFilesql,function(err,a){
        rc(err,a);
      })
    }else{
      rc({error : "date is not null"},null);
    }
    /*var folder_exists = fs.existsSync(config.logFolder());
    if(folder_exists == true){
      var dirList = fs.readdirSync(config.logFolder());
      dirList.forEach(function(fileName){
        console.log(fileName);
        //fs.rmdirSync('C:\\personal\\test\\node\\log\\'+fileName);
       if(Number(fileName) < Number(dataformat)){
         exec('rm -rf' + ' ' + config.logFolder() + fileName);
        }
      })
    }*/
