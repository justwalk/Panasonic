...
Computer.all = function(options, callback) {
  if (options.where.group_id && options.where.group_id != 0 && options.where.group_id != -1) {
  ...
  } else if(options.where.group_id == 0) {
...
else if(options.where.group_id == -1) {
    if (options.where.group_id) {
      rest.getJSON(computersURL + '1', function(err, resp, body) {
        if (err)
          return callback(err);
        var  arr = [];
        arr.push(body);
          Computer.arrayFromCGI(arr, function(err, result) {
            callback(err, result);
          });
      });
    }
...
var mapping = {
    ...
    'OffLineLicense':'online_license_session',
