...
module.exports = function(socket) {
  ...
  socket.on('disk:display', function(req, res) {
    User.checkAuthorization('admin', req, res, function() {
      Disk.display(req,function(err, data) {
        res(err, data);
      });
    });
  });

  socket.on('disk:disksync', function(req, res) {
    User.checkAuthorization('admin', req, res, function() {
      Disk.disksync(req,function(err, data) {
        res(err, data);
      });
    });
  });
  ...
  socket.on('disksync:create', function(req, res) {
    User.checkAuthorization('admin', req, res, function() {
      console.log('disks:create');

      Disk.synccreate(req, function(error, diskCopy) {
        if (!error) {
          socket.emit('disksync:create', diskCopy);
          socket.broadcast.emit('disksync:create', diskCopy);
        }
        res(error, diskCopy);
      });
    });
  ...
  socket.on('disksync:update', function(req, res) {
    User.checkAuthorization('admin', req, res, function() {
      Disk.syncupdate(req, function(error, diskCopy) {
        if (!error) {
          socket.emit('disksync:update', diskCopy);
          socket.broadcast.emit('disksync:update', diskCopy);
        }
        res(error, diskCopy);
      });
    });
